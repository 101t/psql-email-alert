...
CREATE OR REPLACE FUNCTION sendEmail (_from Text, _password Text, smtp Text, port INT, receiver Text, _subject Text, 
			  _as_of_date TEXT[], _employee TEXT[], _ts_day TEXT[], _ts_desc TEXT[], _task_name TEXT[], 
...
MESSAGE_TEMPLATE = """
	<h4> Hello there,</h4>
    <p> 
    This is email is an alert for <b>Time Sheets Alert View.</b> <br> 
    Please check the bellow table, 
    </p>	
    <br>
	<table  style="border: 1px solid #ddd;border-collapse: collapse;">
		<tr>
			<td text-align="center" style="border: 1px solid #ddd;padding: 7px;width: 100px"><h4>as_of_date</h4></td>
...
for i in range(len(_employee)):
	...
	MESSAGE_TEMPLATE = MESSAGE_TEMPLATE + """<td text-align="center" style="border: 1px solid #ddd;padding: 7px;width: 100px">""" + _as_of_date[i] + "</td>"
	MESSAGE_TEMPLATE = MESSAGE_TEMPLATE + """<td text-align="center" style="border: 1px solid #ddd;padding: 7px;width: 100px">""" + _employee[i] + "</td>"
	MESSAGE_TEMPLATE = MESSAGE_TEMPLATE + """<td text-align="center" style="border: 1px solid #ddd;padding: 7px;width: 100px">""" + _ts_day[i] + "</td>"
...
DECLARE _as_of_date TEXT[];
		...
		_unit_amount INT[];
BEGIN
	SELECT ARRAY(SELECT as_of_date FROM v_timesheets_alert) INTO _as_of_date;
	SELECT ARRAY(SELECT employee FROM v_timesheets_alert) INTO _employee;
	SELECT ARRAY(SELECT ts_day FROM v_timesheets_alert) INTO _ts_day;
	SELECT ARRAY(SELECT ts_desc FROM v_timesheets_alert) INTO _ts_desc;
	SELECT ARRAY(SELECT task_name FROM v_timesheets_alert) INTO _task_name;
	SELECT ARRAY(SELECT project_name FROM v_timesheets_alert) INTO _project_name;
	SELECT ARRAY(SELECT amount FROM v_timesheets_alert) INTO _amount;
	SELECT ARRAY(SELECT unit_amount FROM v_timesheets_alert) INTO _unit_amount;
	SELECT sendEmail('SENDER@', 'PASSWORD OR APP_PASSWORD', 'smtp.PROVIDER.com', 587, 'RECIEVER', 'Time Sheets Alert', 
					 _as_of_date, _employee, _ts_day, _ts_desc, _task_name, _project_name, _amount, _unit_amount); 
END $$;
-- SELECT * FROM v_timesheets_alert;
